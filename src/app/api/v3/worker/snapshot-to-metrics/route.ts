...
import { runV3SnapshotToMetricsWorker } from '@/v3/engine/snapshot-to-metrics-worker';
...
async function run(limit: number, leaseSeconds: number, lookbackDays: number) {
    const result = await runV3SnapshotToMetricsWorker(limit, leaseSeconds, lookbackDays);
