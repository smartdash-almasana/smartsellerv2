...
import {
    readV3StorePulse,
    V3ClinicalStatusStoreNotFoundError,
    V3RunHistoryStoreNotFoundError,
} from '@/v3/read-models/store-pulse';
...
export async function GET(request: NextRequest) {
    try {
        ...
        const result = await readV3StorePulse({ tenant_id, store_id });
        return NextResponse.json({ ok: true, ...result }, { status: 200 });
    } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        const status =
            error instanceof V3RunHistoryStoreNotFoundError || error instanceof V3ClinicalStatusStoreNotFoundError
                ? 404
                : 500;
