...
-- 4. display_name on v2_stores (nullable — set on OAuth callback)
...
DO $$
BEGIN
  -- memberships: each user sees only their own rows
  IF NOT EXISTS (
  ...
  -- stores: visible if user has a membership row
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
  ...
  -- clinical_signals: visible if user has a membership for that store
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
  ...
  -- health_scores: visible if user has a membership for that store
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
