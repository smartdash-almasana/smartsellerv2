...
import { runV3SignalsToHealthScoreWorker } from '@/v3/engine/signals-to-health-score-worker';
...
async function run(limit: number, leaseSeconds: number, lookbackDays: number) {
    const result = await runV3SignalsToHealthScoreWorker(limit, leaseSeconds, lookbackDays);
