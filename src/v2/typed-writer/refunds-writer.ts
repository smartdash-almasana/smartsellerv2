...
export async function writeRefundFromDomainEvent(
    ctx: { log: (msg: string) => void },
    domainEvent: DomainEvent
): Promise<{ inserted: number; updated: number; dlq: number }> {
    const result = { inserted: 0, updated: 0, dlq: 0 };

    // Assuming we might map 'refund.updated' for refunds
    if (domainEvent.event_type !== 'refund.updated') {
        return result;
    }
    ...
    try {
        ...
        const refundData: any = {
            ...
            refund_external_id: domainEvent.entity_id,
            raw_jsonb: payload,
            last_occurred_at: domainEvent.occurred_at,
            last_source_event_id: domainEvent.source_event_id
        };

        if (payload.payment_id) refundData.payment_external_id = String(payload.payment_id);
        if (payload.order_id) refundData.order_external_id = String(payload.order_id);
        if (payload.amount !== undefined) refundData.amount = Number(payload.amount);
        if (payload.currency_id) refundData.currency_code = String(payload.currency_id);
        if (payload.date_created) refundData.refunded_at_provider = new Date(payload.date_created as string).toISOString();

        const { error: upsertErr } = await supabaseAdmin
            .from('v2_refunds')
            .upsert(refundData, {
                onConflict: 'provider_key, store_id, refund_external_id',
        ...
        ctx.log(`[DLQ] Sending refund event ${domainEvent.domain_event_id} to DLQ. Error: ${error.message}`);
        await sendToDlq(domainEvent, error, 'typed_writer_refunds', resolvedProviderKey);
        result.dlq += 1;
    }
...
async function sendToDlq(domainEvent: DomainEvent, error: any, source: string, providerKey: string) {
    ...
    if (dlqErr) {
        console.error(`[DLQ] Failed to write refund DLQ entry: ${dlqErr.message}`);
    }
}
